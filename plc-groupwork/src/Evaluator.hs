...
handleDeclaration (ValueDeclaration LangAst.BooleanType name booleanExpression) (Environment dataComponents (VariableList varList) outputBuffer) = 
  case evaluateBooleanCondition (Environment dataComponents (VariableList varList) outputBuffer) booleanExpression of
    ...
    Just (Val (BoolValue b)) -> trace ("evaluated boolean expression: " ++ show b) (Environment dataComponents (VariableList ((BoolVar name b) : varList)) outputBuffer)
...
handleDeclaration (ValueDeclaration LangAst.IntegerType name booleanExpression) (Environment dataComponents (VariableList varList) outputBuffer) = 
  case evaluateBooleanCondition (Environment dataComponents (VariableList varList) outputBuffer) booleanExpression of
    Nothing -> error ("Could not evaluate expression: " ++ show (booleanExpression))
    Just (Val (IntValue i)) -> trace ("evaluated boolean expression: " ++ show i) (Environment dataComponents (VariableList ((IntVar name i) : varList)) outputBuffer)
...
handleDeclaration (ValueDeclaration LangAst.StringType name booleanExpression) (Environment dataComponents (VariableList varList) outputBuffer) = 
  case evaluateBooleanCondition (Environment dataComponents (VariableList varList) outputBuffer) booleanExpression of
    Nothing -> error ("Could not evaluate expression: " ++ show (booleanExpression))
    Just (Val (DataStructure.StringValue s)) -> trace ("evaluated boolean expression: " ++ show s) (Environment dataComponents (VariableList ((StrVar name s) : varList)) outputBuffer)
...
handleDeclaration (ValueDeclaration LangAst.NodeType name booleanExpression) (Environment dataComponents (VariableList varList) outputBuffer) = 
  case evaluateBooleanCondition (Environment dataComponents (VariableList varList) outputBuffer) booleanExpression of
    Nothing -> error ("Could not evaluate expression: " ++ show (booleanExpression))
    Just (Val (NodeValue indices)) -> trace ("evaluated boolean expression: " ++ show (NodeValueDec name indices)) (Environment dataComponents (VariableList ((NodeValueDec name indices) : varList)) outputBuffer)
...
handleDeclaration (ValueDeclaration LangAst.RelationType name booleanExpression) (Environment dataComponents (VariableList varList) outputBuffer) = 
  case evaluateBooleanCondition (Environment dataComponents (VariableList varList) outputBuffer) booleanExpression of
    Nothing -> error ("Could not evaluate expression: " ++ show (booleanExpression))
    Just (Var (RelationVar name indices)) -> trace ("evaluated boolean expression: " ++ show (RelationVar name indices)) (Environment dataComponents (VariableList ((RelationVar name indices) : varList)) outputBuffer)
...
evaluateObjectAttribute (Environment dataComponents _ _) (ObjectAttribute setName attr (setIndex, dataIndex))
  ...
  | otherwise = trace (show (ObjectAttribute "_" attr (setIndex, dataIndex))) resultValue
  where
...
evaluateExpression env (BooleanNot e1) = returnValue
  where
    ...
    returnValue = case maybeValue of
      ...
      _ -> error ("Mismatching types in expression !" ++ show e1)
evaluateExpression env (Dot e1 dotMethod) = returnValue
  where
    maybeValue = evaluateExpression env e1
    returnValue = case maybeValue of
        ...
        case dotMethod of
          Attribute attr -> 
            case attr of
              ...
              n -> Just (Var (HeaderFieldVar attr DataStructure.IntegerType)) -- replace this with fieldVars
              _ -> error ("Invalid Attribute for type relationset")
          Method meth args ->
            case meth of
              "get" -> error ("Undefined Method get") -- -> RelationValueDec
              "hasRelation" -> error ("Undefined meethod hasRelation") -- -> Bool
              _ -> error ("Invalid Method for type relationset: " ++ show meth)
        ...
        case dotMethod of 
          Attribute attr -> 
            case attr of
              ...
              _ -> error ("Invalid Attribute for type node")
          Method meth args ->
            case meth of
              ...
              _ -> error ("Invalid Method for type node")
      ...
      Just (Var (RelationVar name indices)) ->
        case dotMethod of 
          Attribute attr -> 
            case attr of
              n -> Just (Var (ObjectAttribute name attr indices)) -- replace this with fieldVars
              _ -> error ("Invalid Attribute for type relation")
          Method meth args ->
            case meth of
              "hasType" -> error ("Undefined Method hasType")
              _ -> error ("Invalid Method for type relation")
        ...
        case dotMethod of 
          Attribute attr -> 
            case attr of
              ...
              _ -> error ("Invalid Attribute for type node reference")
          Method meth args ->
            case meth of
              _ -> error ("Invalid Method for type node reference")
